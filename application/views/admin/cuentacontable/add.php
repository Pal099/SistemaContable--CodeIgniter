            ...
            if (response.length > 0) {
                const rows = response.map((tipo, index) => {
                    return [
                        ...
                        tipo.descripcion,
                        tipo.tipo,
                        tipo.codigo, // Código de la cuenta padre
                        tipo.id_padre
                    ];
                });
                ...
                $('#TablaTipos tbody').off('click').on('click', 'tr', function () {
                    ...
                    // Llamar a la función para actualizar el código editable
                    actualizarCodigoEditable(codigoSeleccionado, tipoSeleccionado);
...
// Función para permitir editar solo la parte específica del código
function actualizarCodigoEditable(codigoBase, tipoSeleccionado) {
    let codigoInput = document.getElementById("codigo_nuevo");
    let tipoInput = document.getElementById("hiddenNombreTipo"); // Input oculto para el tipo

    // Clonar el input para eliminar eventos previos y evitar acumulaciones
    let nuevoInput = codigoInput.cloneNode(true);
    codigoInput.replaceWith(nuevoInput);
    codigoInput = nuevoInput; // Reasignar la referencia al nuevo input limpio

    let parteFija = "";
    let parteEditable = "_"; // Se mantiene el "_" como indicador
    let cerosRestantes = "";

    switch (tipoSeleccionado.toLowerCase()) {
        case "título":
            parteFija = codigoBase.substring(0, 1);
            parteEditable = ""; // No editable
            cerosRestantes = "00000000000";
            tipoInput.value = "Título";
            break;
        case "grupo":
            parteFija = codigoBase.substring(0, 1);
            cerosRestantes = "0000000000";
            tipoInput.value = "Grupo";
            break;
        case "subgrupo":
            parteFija = codigoBase.substring(0, 2);
            cerosRestantes = "000000000";
            tipoInput.value = "Subgrupo";
            break;
        case "cuenta":
            parteFija = codigoBase.substring(0, 4);
            cerosRestantes = "00000000";
            tipoInput.value = "Cuenta";
            break;
        case "subcuenta":
            parteFija = codigoBase.substring(0, 6);
            cerosRestantes = "0000000";
            tipoInput.value = "Subcuenta";
            break;
        case "analitico1":
            parteFija = codigoBase.substring(0, 8);
            cerosRestantes = "000";
            tipoInput.value = "Analítico 1";
            break;
        case "analitico2":
            parteFija = codigoBase.substring(0, 11);
            cerosRestantes = "";
            tipoInput.value = "Analítico 2";
            break;
        default:
    ...
    // Asignar el valor al input con la parte fija, editable y ceros restantes
    codigoInput.value = `${parteFija}${parteEditable}${cerosRestantes}`;
    codigoInput.setSelectionRange(parteFija.length, parteFija.length + 1);
    codigoInput.focus();

    // Evento para controlar la edición sin modificar la parte fija ni los ceros restantes
    codigoInput.addEventListener("input", function () {
        let valor = codigoInput.value.replace(/\D/g, ""); // Solo números
        if (valor.length < parteFija.length) {
            valor = parteFija; // Si borra parte fija, la restaura
        }

        // Asegurar que la parte editable solo tenga un número y se conserve "_"
        let numeroEditado = valor.substring(parteFija.length, parteFija.length + 1) || "_";

        // Reconstruir el código manteniendo las partes fijas y el "_"
        let nuevoValor = parteFija + numeroEditado + cerosRestantes;
        codigoInput.value = nuevoValor;

        // Volver a posicionar el cursor en la parte editable
        codigoInput.setSelectionRange(parteFija.length, parteFija.length + 1);
    });
}

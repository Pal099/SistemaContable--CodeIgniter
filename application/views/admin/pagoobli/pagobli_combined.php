...
<body>
   ...
   <div class="modal fade" id="modalListaObligacion" tabindex="-1" aria-labelledby="ModalListaObligaciones" aria-hidden="true">
                ...
                <table style="width: 100%;" class="table table-hover" id="TablaListaObligacion">
                    ...
                    <tbody>
                        <?php foreach ($registros_por_proveedor as $asi): ?>
                            ...
                            <tr class="list-item" onclick="selectAsi('<?= $asi->ruc_proveedor ?>', '<?= $asi->razso_proveedor ?>', '<?= $asi->concepto ?>', '<?= $asi->fecha ?>', '<?= $asi->id_pro ?>', '<?= $asi->id_ff ?>', '<?= $asi->id_of ?>', '<?= $asi->codigo ?>', '<?= $asi->descrip ?>',
                                   '<?= $asi->comprobante ?>', '<?= $asi->detalles ?>', '<?= $asi->MontoPago ?>','<?= $asi->Debe ?>',  '<?= $asi->idnumasi ?>', '<?= $asi->cheques_che_id ?>')"
                                ...
                                <td><?= $asi->pagado ?></td>
                                <td><?= $asi->MontoPago ?></td>
                                <td hidden><?= $asi->Debe ?></td>
                                <td hidden><?= $asi->Haber ?></td>
                                <td hidden><?= $asi->IDCuentaContable ?> - <?= $asi->codigo ?> - <?= $asi->descrip ?></td>
                                <td><?= $asi->nombre_fuente ?></td>
                                <td><?= $asi->nombre_programa ?></td>
                                <td><?= $asi->nombre_origen ?></td>
                                <td hidden><?= $asi->detalles ?></td>
                                <td hidden><?= $asi->comprobante ?></td>
                                <td hidden><?= $asi->cheques_che_id ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    <script>
        ...
        function selectAsi(ruc, razonSocial, conceptos, fechas, programas, fuentes, origens, cuentas, descrip, comp, deta, montos, debes, cheq) {
            // Actualizar los campos de texto en la vista principal
            document.getElementById('ruc').value = ruc;
            document.getElementById('contabilidad').value = razonSocial;
            document.getElementById('observacion').value = conceptos;
        ...
        $("#formularioPrincipal").on("submit", function (e) {
            ...
            $("#miTabla tbody tr:gt(0)").each(function (e) {
                ...
                var fila = {
                    ...
                    descripcion_cc_2: $(this).find("input[name='descripcion_cc_2']").val(),
    ...
    <script>
        ...
        document.getElementById('Debe').addEventListener('input', function () {
            ...
            // y actualiza automáticamente el campo 'haber' a 0 cada vez que 'debe' cambia.
                ...
                document.getElementById('MontoPago_2').value = 0;
